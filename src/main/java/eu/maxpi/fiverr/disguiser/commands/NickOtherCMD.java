...
public class NickOtherCMD implements CommandExecutor {
    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if(!sender.hasPermission("disguiser.nickother")){
            sender.sendMessage(PluginLoader.lang.get("no-permission"));
            return true;
        }

        if(args.length != 2){
            sender.sendMessage(PluginLoader.lang.get("nick-other-usage"));
            return true;
        }
        ...
        try {
            NMSHelper.setName(p, ColorTranslator.translate(args[1]));
